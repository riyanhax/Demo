...
function drawThreeGeo(json, radius, shape, materalOptions, container) {
    ...
    var json_geom = createGeometryArray(json);    //An array to hold the feature geometries. 用于保存特征几何图形的数组。
    var convertCoordinates = getConversionFunctionName(shape);   //Whether you want to convert to spherical or planar coordinates.是否要转换为球面坐标或平面坐标
        ...
        if (json_geom[geom_num].type == 'Point') {
            ...
            for (var polygon_num = 0; polygon_num < json_geom[geom_num].coordinates.length; polygon_num++) {
                for (var segment_num = 0; segment_num < json_geom[geom_num].coordinates[polygon_num].length; segment_num++) {
                    ...
                    drawLine(y_values, z_values, x_values, materalOptions);
                   // drawShape(y_values, z_values, x_values, materalOptions);
    ...
    function drawShape(x_values, y_values,options) {

        var californiaPts = [];
      //  var pointsX,pointsY,pointsZ;
        createVertexForEachPointShape(californiaPts, x_values, y_values);
       ...
       // for( var i = 0; i < californiaPts.length; i ++ ) californiaPts[ i ].multiplyScalar( 0.25 );
        ...
        console.log(pointsX);
        addLineShape(californiaShape, 0xf08000, 0, 0, 0, 0, 0, 0, 3 );
    ...
    function createVertexForEachPointShape(object_geometry, values_axis1, values_axis2) {
        for (var i = 0; i < values_axis1.length; i++) {
            object_geometry.push(new THREE.Vector3(values_axis1[i],values_axis2[i]));
            pointsX = values_axis1[0];
            pointsY = values_axis2[0];

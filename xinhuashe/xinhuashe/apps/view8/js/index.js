...
function chartTop(ops){
	...
	function drawData(i,ops){
		...
		var descText = new PIXI.Text(ops.desc,{fontSize:20,fill:ops.color});
		descText.x = 176+i*455;
		descText.y = 136;
		app.stage.addChild(descText);

		var dataNum = new PIXI.Text(0,{fontSize:50,fill:'#fff'});
		dataNum.x = 176+i*455;
		dataNum.y = 60;
		app.stage.addChild(dataNum);

		var unitText = new PIXI.Text(ops.unit,{fontSize:25,fill:'#fff'});
...
function chartBottom(data){
	...
	function drawContent(i,ops){
		...
		var title = new PIXI.Text(ops.title,{fill:'#fff',fontSize:20});
		title.x = 73+i*247;
		title.y = 55;
	...
	function drawData(i,index,title,num,color){
		var text = new PIXI.Text(title,{fill:color,fontSize:18});
		text.x = 65+index*247;
		text.y = 133+i*163;
		app.stage.addChild(text);

		var num = new PIXI.Text(num,{fill:'#fff',fontSize:36});
		num.x = 65+index*247;
		num.y = 173+i*163;
		app.stage.addChild(num);

		var unit = new PIXI.Text('次',{fill:'#fff',fontSize:20});

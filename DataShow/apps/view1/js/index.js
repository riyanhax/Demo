...
;(function(window,document,undefined){
	...
	function chartRight(data,res){
		...
		for(var i=0;i<fileds.length;i++){
			...
			count = transFiveNum(count);
			size = size.toString();
			while(size.length<4){
				size = '0'+size
			}
			size = size.substr(0,2)+'.'+size.substr(2);
			console.log(i,size)
			var aLi = oFirst.find('li');
			$(aLi).each(function(i,o){
				if(!$(this).hasClass('separate')){
					$(this).find('.dowebok').attr('data-end',count[i]);
				}
			});

			var aSizeLi = oLast.find('li');
			$(aSizeLi).each(function(i,o){
				if(!$(this).hasClass('separate')){
					$(this).find('.dowebok').attr('data-end',size[i]);
				}
			});
	...
	function flop(ele,ops){
		...
		function ani(obj){
			...
			var timer = setInterval(function(){
				...
				//$(aTop[index]).css({'zIndex':zIndex/*,'transform':'rotateX(-90deg)'*/}).addClass('rotate90');
				/*setTimeout(function(){
					$(aBot[index-1]).css({'zIndex':zIndex,'transform':'rotateX(0deg)'}).addClass('rotate');
					index--;
				},100)*/

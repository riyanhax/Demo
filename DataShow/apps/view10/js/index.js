...
$.get(Config.api.api3).success(function(res){
	...
	for(var i=0;i<data.length;i++){
		xAis.push(data[i].hour);
		chart.push(data[i].number);
	}
	barChart(ctx,{
		x:108,
		y:97,
...
$.get(Config.api.api5).success(function(res){
	...
	for(var i=0;i<chart.length;i++){
		chart[i].text = data[i].TAG;
		chart[i].value = total==0?0:data[i].number/total*230;
	}
...
function userChartLeft(){
	...
	$.get(Config.api.api1).success(function(res){
		...
		for(var i=0;i<data.length;i++){
			list[i].name = data[i].province;
			list[i].value = data[i].number;
		}
	...
	function static() {
	    var r1 = new PIXI.Sprite.fromImage('./imgs/r1.png');
	    ...
	    var r2 = new PIXI.Sprite.fromImage('./imgs/r2.png');
	    ...
	    if (init == 0) {
	        ...
	        for (var i = 0; i < angles.length; i++) {
	           ...
	           /* paint.beginFill(d.color, 1);
	            ...
	            var text = new PIXI.Text(Math.round(d.value),style);
	...
	$.get(Config.api.api2).success(function(res){
		...
		for(var i=0;i<data.length;i++){
			dataList[i].name = data[i].groupName;
			dataList[i].value = (data[i].number/total);
		}
	});
	typeclass(dataList);

	function typeclass(dataList){

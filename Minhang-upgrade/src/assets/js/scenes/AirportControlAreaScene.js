App.AirportControlAreaScene = function(){
    ...
    if(this.flag){
      ...
      this.timeIndex = setInterval(function(){
        ...
        if(selectAreas && selectAreas.length){
      	...
      	console.log('ControlArea update');
      },60000);
    //  controlAreaStatus('low');
      this.flag = false;
    }
  ...
  function getDefaultSection(){
    ...
    $.get(path+'get-sectionsave').done(function(res){
  ...
  function createSectorPolygon(data,delaydata){
    ...
    // var isAddedEntites = App.viewer.entities.getById(data.sectorname);
    // if(isAddedEntites){
    //   isAddedEntites.material = Cesium.Color.fromBytes(colors[0],colors[1],colors[2],colors[3])
    // }else{
      App.viewer.entities.add({
        name : 'controlArea-'+data.heightLevel,
        id : data.sectorname,
        polygon : {
          hierarchy : {//fromDegreesArray

...
App.AirportOperSituAndMeteoInfoScene = function() {
  ...
  this.show = function() {
    if (!this.flag) {
      ...
      this.timeindex = setInterval(function() {
        delete airport;
        airport = App.Preload.assets.airport;
        var airportinfo = App.Preload.assets.airportinfo[0];
        deleteAirportSceneIcon();
        App.viewer.entities.values = [];
        initAirportSceneIcon();
        if(isShowAll){
          window.sectionAndAirportShow(isShowAll,[]);
        }
        //hideMeteoIonfoScene()
        if ($('.title').eq(0).is(':hidden')) {
          showMeteoInfoScene()
        } else {
          if (isShowDefaiultAirport) {
            setDefaultAirportOperIcon();
        ...
        initCapitals();
        updateOnDutyPeople();
        $('.airportLeftMenu .leftItem:eq(8) .middleText:eq(1)').text(timeType[time_set_index]).parent().find('.rightNum').text(airportinfo[strType[time_set_index]]);
        console.log('Airport update');
      }, intervalTime);
      this.flag = true;
    }
  }.bind(this);
  ...
  function showAlert(apName) {
    $.get('http://' + SERVER.ip + ':' + SERVER.port + '/get_airport', {
      apName: apName
    }).done(function(res) {
      var data = JSON.parse(res).data[0]
      $('.airport .desc:eq(1) strong').text(data.depCapacity);
    })
    $.get('http://' + SERVER.ip + ':' + SERVER.port + '/get_airportdynamic', {
      apName: apName
    }).done(function(response) {
      var res = JSON.parse(JSON.parse(response).data[0].currencyStasticsResult);
      var res2 = JSON.parse(JSON.parse(response).data[0].countStasticsResult);
      var key = parseTime();
    ...
    $.get('http://' + SERVER.ip + ':' + SERVER.port + '/get_weather', {
      apName: apName
    }).done(function(res) {
      res = JSON.parse(res);
      var data = res.data[0];
      ...
      $('.airport_info_layer .weather .desc:eq(0) strong').text(status[data.weatherStatus])
      $('.airport_info_layer .weather .desc:eq(1) strong').text(data.HVisibility)
      $('.airport_info_layer .weather .desc:eq(2) strong').text(data.windSpeed)
    });
  }
 ...
 // set visibile about airport weather
  ...
  window.switchsomeInfo = function(arg){
    if(arg== 'isShow'){
      $('.leftItem:eq(3)').show()
      $('.leftItem:eq(4)').show()
      $('.hline:eq(0)').show();

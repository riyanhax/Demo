...
SViewUI.prototype.onresize = function()
{
	...
	this.bulidChart("chartPanel",this.data);
...
SViewUI.prototype.redrawYAxis = function(jObj)
{
	...
	if (this.isYear())
	{
		...
		var isHave = this.filterData(dataList[date.Format("yyyyMM")]);
		if(!isHave){
			date.setMonth(date.getMonth() - 1);
		}
		for (var i = 0; i < 12; i++)
...
SViewUI.prototype.setJsonData = function(jObj)
{
	...
	if (this.isYear())
	{
		...
		var isHave = this.filterData(dataList[date2.Format("yyyyMM")]);
		date.setDate(1);
		...
		if (this.monthPanel != null && !this.isYear())
		{
			...
			for (var i = 0; i < 13; i++)
			{
				...
				if (i != 0)
				{
					...
					if (month == this.month)
					{
						cell.style.cssText += "border:" + this.GI.monthPanel_border + ";border-bottom:none;border-top-left-radius:" + radius + "px;border-top-right-radius:" + radius + "px;";
...
SViewUI.prototype.bulidChart = function(container,data)
{
	var dataList = data.rateList;	
	if (this.isRate())
	{
	...
	xAxis.labels.formatter = function()
	{
		...
		var isHave = rv.filterData(dataList[mydate.Format("yyyyMM")]);
		if(!isHave){
			this.value --;
		}
		if (rv.isYear())
		{
			var d = new Date();
			d.setDate(1);
			d.setMonth(d.getMonth() - 13 + this.value);
		...
		var html = "<span  style='font-weight:bold;font-size: " + rv.GI.chart_xAxis_fontSize + "px;color: #ffffff;font-family: times new roman'>" + text + "</span>";
...
SViewUI.prototype.setView = function(dataType, timeType, year, month, area, airport, title,data)
{
	// 记录值
	...
	this.bulidChart("chartPanel",data);
...
function GeoInfo()
{
	...
	this.chartPanel_height = this.bgPanel_height - this.titlePanel_height - this.titlePanel_paddingTop - this.titlePanel_paddingBottom - this.titlePanel_borderWidth - this.infoLine_margin * 3 - this.monthPanel_height;
...
GeoInfo.prototype.getGeoInfo = function(w, h)
{
	...
	[ parseInt(this.chart_xAxis_margin[0] * gi.sizeRateH), parseInt(this.chart_xAxis_margin[1] * gi.sizeRateW), parseInt(this.chart_xAxis_margin[2] * gi.sizeRateH), parseInt(this.chart_xAxis_margin[3] * gi.sizeRateW) ];

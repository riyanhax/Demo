...
RViewUI.prototype.setJsonData = function(jObj, isResize)
{
	...
	// 已起飞架次
	this.setInfoValue("rdepFlightsCount", this.filterData(staMaster.executeDepFlihgtsCount));
	// 已降落架次
	this.setInfoValue("rarrFlightsCount", this.filterData(staMaster.executeDepFlihgtsCount));
	// 排队架次
	this.setInfoValue("unDepCLDFlightsCount", this.filterData(staMaster['cld'+waitType[waitIndex].type+'FlightsCount']));
	// 延误架次

	this.setInfoValue("delay", staMaster.delayFlightsCount);
	// 取消架次
	this.setInfoValue("cnlflightsCount", this.filterData(staMaster.delayAvgTime));
	// 专机架次
	...
	// 实时正常率
	this.setInfoValue("dynamicNormalRate", Math.round(staMaster.scheduleOnTimeRate*100)+ "%");
	this.redrawYAxis(jObj);
...
RViewUI.prototype.sized = function()
{
	...
	try
	{
		...
		this.infoPanel.style.height =  "670px";//this.GI.infoPanel_height +
		...
		this.addFlyInfo();
		this.addInfo("specialFlightsCount", "专机计划班次", "", "#FED038", false);
		this.addInfo("vipFlightsCounts", "要客计划班次", "", "#FED038", true);
		this.addInfo("rdepFlightsCount", "离港计划已执行", "", "#FED038", false);
		this.addInfo("rarrFlightsCount", "到港计划已执行", "", "#FED038", false);
		this.addInfo("delay", "起飞延误班次", "", "#FED038", false);
		this.addInfo("unDepCLDFlightsCount", "关舱门后等待班次", "("+waitType[waitIndex].name+")", "#FED038", true);
		this.addInfo("dynamicNormalRate", "计划航班正常率", "", "#FED038", false);

		this.addInfo("cnlflightsCount", "平均延误时间(分钟)", "", "#FED038", false);
...
RViewUI.prototype.addFlyInfo = function()
{
	...
	c23.align = "center";
	c23.vAlign = "middle";
	c23.innerHTML += "<span style='color:#9AC0D5;font-size:" + this.GI.infoPane_sub_fontsize + "px'>(国际)</span><br>";
...
RViewUI.prototype.setTitle = function(title, date, area)
{
	...
	if (area == null || area == "null")
	{
		...
		this.setInfoTitle("specialFlightsCount", "专机计划班次");
		this.setInfoTitle("vipFlightsCounts", "要客计划班次");
		this.setInfoTitle("cnlflightsCount", "平均起飞延误时间(分钟)");
		this.setInfoTitle("unDepCLDFlightsCount", "关舱门后等待班次", "("+waitType[waitIndex].name+")");
